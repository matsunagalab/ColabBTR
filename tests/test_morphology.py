...
class TestDilationEquivalence:
    ...
    def tip_and_surface(self):
        ...
        surface = torch.randn(20, 20, dtype=torch.float64)
        tip = torch.zeros(7, 7, dtype=torch.float64)
        tip = define_tip(tip, resolution_x=1.0, resolution_y=1.0, probeRadius=3.0, probeAngle=0.3)
        return surface, tip
    ...
    def test_dilation_boundary_no_wraparound(self):
        ...
        surface = torch.zeros(10, 10, dtype=torch.float64)
        surface[9, 9] = 100.0  # large value in bottom-right corner
        tip = torch.zeros(3, 3, dtype=torch.float64)
        result = idilation(surface, tip)
        # With -inf padding, top-left corner should NOT see the bottom-right value
        assert result[0, 0].item() == pytest.approx(0.0)

    def test_small_tip(self):
        """Test with 3x3 minimum tip size — interior must match."""
        torch.manual_seed(0)
        surface = torch.randn(10, 10, dtype=torch.float64)
...
class TestTranslateTipMean:
    def test_already_centered(self):
        """Symmetric tip should stay roughly the same."""
        tip = torch.zeros(11, 11, dtype=torch.float64)
        tip = define_tip(tip, 1.0, 1.0, 5.0, 0.3)
        result = translate_tip_mean(tip)
        # Should be close to original since tip is symmetric
    ...
    def test_all_equal(self):
        ...
        tip = torch.full((7, 7), -5.0, dtype=torch.float64)
        result = translate_tip_mean(tip)
        # Should not crash; weight fallback fills with 1.0
        assert result.shape == tip.shape

    def test_shifted_tip(self):
        """Off-center tip should be re-centered."""
        tip = torch.full((11, 11), -10.0, dtype=torch.float64)
        tip[1, 1] = 0.0  # peak in top-left
        result = translate_tip_mean(tip)
        # After translation, the peak should be closer to center
        xc, yc = compute_xc_yc(tip)
...
class TestDefineTip:
    ...
    def test_non_positive(self):
        ...
        tip = define_tip(tip, 0.5, 0.5, 3.0, 0.5)
        assert (tip <= 0).all()
...
class TestAfmize:
    def test_output_shape(self):
        ...
        tip = torch.zeros(5, 5, dtype=torch.float64)
        tip = define_tip(tip, 0.5, 0.5, 2.0, 0.3)
        config = {
            "min_x": -3.0, "max_x": 3.0,
            "min_y": -3.0, "max_y": 3.0,
            "resolution_x": 0.5, "resolution_y": 0.5,
        ...
        expected_w = len(torch.arange(-3.0, 3.0, 0.5))
        expected_h = len(torch.arange(-3.0, 3.0, 0.5))
...
class TestMathematicalProperties:
    def test_dilation_erosion_roundtrip(self):
        ...
        torch.manual_seed(42)
        surface = torch.randn(20, 20, dtype=torch.float64).abs()
        tip = torch.zeros(5, 5, dtype=torch.float64)
        tip = define_tip(tip, 1.0, 1.0, 2.0, 0.3)
        image = idilation(surface, tip)
        image2 = idilation(ierosion(image, tip), tip)
        # With correct -inf/+inf padding, roundtrip should be exact
    ...
    def test_erosion_reduces_values(self):
        ...
        torch.manual_seed(42)
        image = torch.randn(20, 20, dtype=torch.float64).abs() + 1.0
        tip = torch.zeros(5, 5, dtype=torch.float64)
        tip = define_tip(tip, 1.0, 1.0, 2.0, 0.3)
    ...
    def test_dilation_increases_values(self):
        ...
        # With non-positive tip, dilation(s, t) >= s
